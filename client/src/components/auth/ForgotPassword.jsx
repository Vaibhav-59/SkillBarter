    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <div className={`rounded-3xl p-8 max-w-sm w-full shadow-2xl border relative overflow-hidden ${d ? "bg-gradient-to-br from-[#0d1120] to-[#060912] border-indigo-500/30" : "bg-white border-indigo-200"}`}>
        ...
        <div className="text-center relative z-10">
          ...
          <h3 className="text-3xl font-black bg-gradient-to-r from-indigo-400 to-violet-500 bg-clip-text text-transparent mb-3">Password Updated! 🔐</h3>
          <p className={`text-lg mb-6 ${d ? "text-slate-300" : "text-slate-600"}`}>Your password has been successfully reset. Redirecting to login...</p>
...
export default function ForgotPassword() {
  ...
  const validateField = (name, value) => {
    ...
    if (name === "password") {
      ...
      else error = validateStrongPassword(value);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/auth/direct-reset-password`, {
  ...
  // Identical input style system as LoginForm, RegisterForm, ResetPassword
  const inputBase = d
  ...
  return (
    <div className={`min-h-screen relative flex items-center justify-center p-4 transition-colors duration-500 overflow-hidden ${d ? "bg-[#060912]" : "bg-slate-50"}`}>
      ...
      <div className={`relative z-10 w-full max-w-5xl lg:min-h-[580px] grid lg:grid-cols-2 rounded-3xl overflow-hidden transition-all duration-500 ${d ? "shadow-[0_0_80px_-20px_rgba(99,102,241,0.3)]" : "shadow-2xl shadow-slate-300/50"}`}>
        ...
        <div className="relative hidden lg:flex flex-col justify-center items-center p-14 overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700">
          ...
          <div className="relative z-10 text-center max-w-xs">
            ...
            <h1 className="text-4xl font-black text-white mb-4 leading-tight">Recover Access.</h1>
            <p className="text-white/80 text-lg leading-relaxed mb-10">
              Enter your email address and securely reset your password instantly.
            </p>
            <div className="space-y-3 text-left">
              {[
                { icon: "🔐", text: "Instant password update" },
                { icon: "⚡", text: "No email verification needed" },
                { icon: "🛡️", text: "Secure password encryption" },
              ].map(({ icon, text }) => (
            ...
            <div className="mt-10 pt-8 border-t border-white/20">
              <p className="text-white/60 text-sm">Remembered your password?</p>
